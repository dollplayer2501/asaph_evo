...
<form action="<?php echo Asaph_Config::$absolutePath; ?>admin/" method="post">
	...
	<dl>
		...
		<dt>Created:</dt>
		<dd>
			<input type="text" name="created" id="inputCreated" value="<?php echo date( 'Y-m-d H:i:s', $post['created']);?>"/>
		...
		<?php if( !empty($post['image']) ) { ?>
			...
			<dt>Site:</dt>
			<dd>
				<input type="text" name="source" class="long" value="<?php echo $post['source']; ?>"/> <a href="<?php echo $post['source']; ?>">go to site</a>
			</dd>
		<?php } else { ?>
			<dt>Text:</dt>
		...
		<dt>Memo:</dt>
		<dd>
			<textarea id="memo" name="memo"><?php echo $post['memo']; ?></textarea>
		</dd>
		<dt>Tag:</dt>
		<dd>
			<input type="text" name="tag" class="long" value="<?php echo $post['tag']; ?>"/>
		</dd>
		<dt></dt>
		<dd>
