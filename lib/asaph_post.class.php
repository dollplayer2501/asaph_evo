...
class Asaph_Post extends Asaph_Admin {
	...
	public function postUrl( $url, $title, $memo, $tag ) {
		if( !$this->userId ) {
		...
		$this->db->insertRow( ASAPH_TABLE_POSTS, array(
			...
			'title' => trim($title),
			'memo' => $memo,
			'tag' => $this->composeTag( $tag )
		));
		return true;
	}
	
	
	public function postImage( $url, $referer, $title, $memo, $tag ) {
		if( !$this->userId ) {
			return 'not-logged-in';
		}
	...
	private function composeTag( $tag ) {
		$tags = explode( ',', $tag );
		sort( $tags );
		$tagwk = '';
		$i = 0;
		foreach ( $tags as $item ) {
			$tagwk .= trim($item).( $i < count($tags) - 1 ? ',' : '' );
			$i++;
		}
		return $tagwk;
	}
