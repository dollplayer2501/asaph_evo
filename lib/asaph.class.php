...
class Asaph {
	...
	public function getPosts( $page, $tag = null ) {
		...
		if ( strlen($tag) == 0 ) {
			$posts = $this->db->query( 
				'SELECT SQL_CALC_FOUND_ROWS
					...
					p.id, p.source, p.thumb, p.image, p.title, p.tag, u.name AS user
				FROM 
					'.ASAPH_TABLE_POSTS.' p
	...
	public function getTagCloud( ) {
		...
		foreach( $posts as $field ) {
			if (strlen(trim($field['tag'])) == 0) {
				$none++;
			} else {
				$tags = explode( ',', $field['tag'] );
				foreach ( $tags as $item ) {
					if ( isset($tagcloud[$item]) ) {
						$tagcloud[$item]++;
					} else {
						$tagcloud[$item] = 1;
					}
	...
	public function getSinglePost( $id ) {
		...
		$post = $this->db->getRow( 
			'SELECT 
				UNIX_TIMESTAMP(p.created) as created, 
				p.id, p.source, p.thumb, p.image, p.title, p.memo, p.tag, u.name
			FROM 
				'.ASAPH_TABLE_POSTS.' p
			LEFT JOIN '.ASAPH_TABLE_USERS.' u 
				ON u.id = p.userId
